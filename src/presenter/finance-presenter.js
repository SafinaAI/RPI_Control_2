...
export default class FinancePresenter {
  ...
  _initializeHandlers() {
    ...
    const typeFilterElement = this.filtersComponent
      .getElement()
      .querySelector("#type-filter");
    const categoryFilterElement = this.filtersComponent
      .getElement()
  ...
  _handleFilterChange() {
    const type = this.filtersComponent
      .getElement()
      .querySelector("#type-filter").value;
    const category = this.filtersComponent
      .getElement()
      .querySelector("#category-filter").value;

    this.model.setFilters(type, category);
    this._updateView();
  }
  ...
  _handleFormSubmit(evt) {
    ...
    if (!type || !category || isNaN(amount) || amount <= 0) {
      alert("Заполните все поля корректно! Сумма должна быть положительной.");
      return;
    }

    const operation = { type, category, amount };
    this.model.addOperation(operation); // Добавление операции в модель
    this._updateView(); // Обновление представления
